...
namespace ExampleObjectDetection
{
    class Program
    {
        ...
        private static string _input = @"C:\Users\wangxi\Desktop\ComputerVision\test.jpg";
        ...
        static void Main(string[] args)
        {
            ...
            var modelFile = @"C:\Users\wangxi\Desktop\ComputerVision\frozen_inference_graph.pb";
            ...
            using (var graph = new TFGraph())
            {
                ...
                using (var session = new TFSession(graph))
                {
                    ...
                    foreach (var tuple in fileTuples)
                    {
                        ...
                        var masks = (float[,,,])output[3].GetValue(jagged: false);
                        var num = (float[])output[4].GetValue(jagged: false);

                        Console.WriteLine($"Boxes {boxes.GetLength(1)}");
                        Console.WriteLine($"scores {scores.GetLength(1)}");
                        Console.WriteLine($"classes {classes.GetLength(1)}");
                        Console.WriteLine($"masks {masks.GetLength(1)}");
            ...
            //{
                for (int i = 0; i < x; i++)
                {
                    for (int j = 0; j < y; j++)
                    {
                        ...
                        Console.WriteLine($"({xmin},{xmax})({ymin},{ymax}) : {(scores[i, j] * 100).ToString("0")}%");
